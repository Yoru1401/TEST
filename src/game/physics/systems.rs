...
use crate::game::physics::config::SKIN_WIDTH;
...
pub fn accumulate_forces(
    ...
    for (entity, mut vel, config, mut force_app, tension, spring, ground_state) in &mut bodies {
        ...
        if let Some(&TensionAnchor(anchor)) = tension {
...
pub fn detect_ground(
    ...
    for (entity, mut ground_state, _config) in &mut bodies {
        ...
        if let Some(hit_data) = hit {
            ground_state.ground_normal = hit_data.normal1.into();
        }
    }
}
...
pub fn resolve_collisions(
    ...
    for (entity, mut vel, _physics_config, mut contacts, mut transform) in &mut bodies {
        ...
        for _ in 0..move_config.move_and_slide_iterations {
            ...
            let hit_point = sweep_hit.point2.into();
